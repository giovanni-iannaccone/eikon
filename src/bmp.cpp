...
int Handler::get_dimensions(std::istream &file, uint *height, uint *width) {
    ...
    *width  = utils::le::get_bytes<uint>(file);
    *height = utils::le::get_bytes<uint>(file);
...
Data Handler::read_info_header(std::istream &file, uint *height_ptr, uint *width_ptr) {
    Data bmpdata;

    utils::le::get_bytes<uint32_t>(file);
    
    *width_ptr  = utils::le::get_bytes<uint32_t>(file);
    *height_ptr = utils::le::get_bytes<uint32_t>(file);
...
void Handler::write_info_header(std::ostream &file, uint height, uint width, Data *bmp) {
    utils::le::write_as_bytes(file, 0x28);

    utils::le::write_as_bytes(file, width);
    utils::le::write_as_bytes(file, height);

    utils::le::write_as_bytes(file, 0x180001);
    
    if (bmp != nullptr) {
        utils::le::write_as_bytes(file, bmp->compression);
        utils::le::write_as_bytes(file, 3 * height * width);
