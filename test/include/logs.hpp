...
namespace logs {
    ...
    static constexpr
    void log(logType lt, const std::format_string<Args...> fmt, Args&&... args) noexcept
    {
        std::print(std::cout, "{}", lt);
        std::print(std::cout, fmt, std::forward<Args>(args)...);
        std::print(std::cout, "{}", logs::Type::INFO);
    }

    constexpr
    void failure_logs(std::string_view function_name) noexcept
    {
    ...
    constexpr
    void info_logs(std::string_view msg) noexcept
    {
        logs::log(logs::Type::INFO, "{}\n", msg);
    }

    constexpr
    void newfile_logs(std::string_view function_name) noexcept
    {
        logs::log(logs::Type::NEW_FILE, "{} file is new\n", function_name);
    }

    constexpr
    void success_logs(std::string_view function_name) noexcept
    {
